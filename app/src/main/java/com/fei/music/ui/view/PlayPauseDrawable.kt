...
class PlayPauseDrawable : Drawable {
    ...
    override fun draw(canvas: Canvas) {
        ...
        val barDist =
            lerp(mPauseBarDistance, 0f, mProgress)
        // The current width of each pause bar.
        val barWidth =
            lerp(mPauseBarWidth, mPauseBarHeight / 2f, mProgress)
        // The current position of the left pause bar's top left coordinate.
        val firstBarTopLeft =
            lerp(0f, barWidth, mProgress)
        // The current position of the right pause bar's top right coordinate.
        val secondBarTopRight =
            lerp(2 * barWidth + barDist, barWidth + barDist, mProgress)
        ...
        val startingRotation: Float = if (isPlay) 90f else 0f
        canvas.rotate(
            lerp(
                startingRotation,
                startingRotation + 90,
